...
public class ExcelFileServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (params.length == 2) {
            ...
            // flightEnties = flightEntryDAO.listflightEntry(Integer.parseInt(params[1]), Integer.parseInt(params[2])); // TODO -
            // what for is this?
            // month
        ...
        if (flightEnties != null) {
            String sheetname = String.valueOf(year) + (month == -1 ? "" : String.format("%02d", month + 1))
                    + (day == -1 ? "" : String.format("%02d", day)) + "-" + place.replace(" ", "_");
