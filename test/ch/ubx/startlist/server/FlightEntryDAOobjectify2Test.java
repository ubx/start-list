...
public class FlightEntryDAOobjectify2Test {
    ...
    private static FeGenDAOobjectify<FeDate, FePlace, Long> dateDAO;
    private static FeGenDAOobjectify<FeStore, FeStore, String> storeDAO;
    private static FeGenDAOobjectify<FeYear, FeStore, Long> yearDAO;
    private static FeGenDAOobjectify<FePlace, FeYear, String> placeDAO;

    @Before
    public void setUp() throws Exception {
        helper.setUp();
        flightEntryDAOobjectify = new FlightEntryDAOobjectify2();
        dateDAO = new FeGenDAOobjectify<FeDate, FePlace, Long>(FeDate.class);
        storeDAO = new FeGenDAOobjectify<FeStore, FeStore, String>(FeStore.class);
        yearDAO = new FeGenDAOobjectify<FeYear, FeStore, Long>(FeYear.class);
        placeDAO = new FeGenDAOobjectify<FePlace, FeYear, String>(FePlace.class);
    }
    ...
    public void testRemoveFlightEntry() {
        ...
        List<FlightEntry> flightEntries = flightEntryDAOobjectify.listflightEntry(date, 0, 1000);
    ...
    private void createFlightEntries(Long[] dates, String[] places, int cnt) {
        ...
        for (int i = 0; i < dates.length; i++) {
            for (int j = 0; j < places.length; j++) {
                for (int k = 0; k < cnt; k++) {
                    FlightEntry fe = new FlightEntry(PILOT, dates[i] + k, dates[i] + k + 3600000, false, REMARKS, places[j]);
